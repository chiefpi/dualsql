...
class SQLNet(nn.Module):
    ...
    def forward(self, q, col, col_num, pred_entry,
        ...
        sel_score = self.sel_pred(x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len, gt_sel=gt_sel)
    ...
    def loss(self, score, truth_num, pred_entry):
        ...
        sel_num_truth = [x[5]-1 for x in truth_num] #might need to be the length of the set of columms
        ...
        gby_num_truth = [x[7] for x in truth_num]
        ...
        ody_num_truth = [x[10] for x in truth_num]
        ...
        # Evaluate parity
        ody_par_truth = [x[11] for x in truth_num]
    ...
    def gen_sql(self, score, col_org, schema_seq):
        ...
        for b in range(B):
            ...
            if -1 in list(cur_tables.keys()):
                del cur_tables[-1]

            table_alias_dict, ret = self.gen_from(list(cur_tables.keys()), schema)
            if len(table_alias_dict) > 0:
                col_map = {}
                for tid, aid in list(table_alias_dict.items()):
